...
export default class AuthorInfo extends React.Component {
    ...
    componentDidMount(){
        const author_id = '苏轼'
        net_work.require('getAuthorInfo', {author_id: author_id})
        .then(data=>{
            // console.log(data)
    ...
    render() {
        const {poteries, BasicInfo, PersonAddresses, PersonAliases} = this.state
        return (
            <div style={{ position: 'relative', width: '90%', left:'5%',top: '2.5%',height: '92%', overflowX: 'auto', overflowY: 'hidden' }}>
                <div className='scroll_container' style={{ position: 'relative', width: 1000, height: '100%',  overflowX: 'hidden', overflowY: 'hidden' }}>
                    {/* 基本信息 */}
                    <div style={{ position: 'relative', width: 300, height: '100%', float: 'left' }}>
                        <img src={author_png} style={{top: '28%', left: 0, maxHeight: '50%', maxWidth: 80,position: 'absolute', float: 'left'}} alt='词人画像'/>
                        <div style={{position: 'absolute', top: '40%',textAlign: 'left', writingMode: 'vertical-lr', left: 96, fontSize: 35}}>{BasicInfo.ChName}</div>
                        <span style={{position: 'absolute', top: '37%',textAlign: 'left', left: 172, fontSize: 10}}>
                    ...
                    {/* 诗词列表 */}
                    <div style={{ position: 'relative', width: 250, height: '80%', marginTop: '5%',float: 'left', overflowY: 'auto' }}>
